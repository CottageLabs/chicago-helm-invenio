...
- **Pro flat-rate plan** ($15/month): once a month of CloudFront request
  numbers is available, compare with the plan's 10M requests. It requires a
  WAF web ACL on the distribution. See
  [aws-cost-reductions.md](aws-cost-reductions.md#cloudfront-planned).
...
| Date | Step | Details |
|---|---|---|
| 25 Sep 2026 | 1 | Certificate `…62932abb-7ab9-4767-b8fb-be366ef6814b` (us-east-1) issued immediately; both validation records were already published. An older us-east-1 request for `knowledge.uchicago.edu` alone is in `VALIDATION_TIMED_OUT` and unused |
